...
import { UserWithDetails, TrainingProgram, TrainingSession, Exercise, TrainingExercise, ExerciseParameter } from '../../types/database';
import { Search, AlertCircle, ChevronDown, ChevronUp, X, Calendar, Camera, Scale, Target, Info, DumbbellIcon } from 'lucide-react';
...
const AdminUsers = () => {
  ...
  const [filterExpiringProgram, setFilterExpiringProgram] = useState<boolean>(false);
  const [filterExpiringSubscription, setFilterExpiringSubscription] = useState<boolean>(false);
  const [selectedUser, setSelectedUser] = useState<any>(null);
  const [userDetails, setUserDetails] = useState<any>(null);
  const [weightHistory, setWeightHistory] = useState<any[]>([]);
  const [progressPhotos, setProgressPhotos] = useState<any[]>([]);
  const [selectedTimeRange, setSelectedTimeRange] = useState<'1m' | '3m' | '6m' | '1y' | 'all'>('all');
  const [userProgram, setUserProgram] = useState<TrainingProgramWithDetails | null>(null);
  const [isEditingProgram, setIsEditingProgram] = useState(false);
  const [editedProgram, setEditedProgram] = useState<TrainingProgramWithDetails | null>(null);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditedProgram({ ...editedProgram, total_weeks: parseInt(e.target.value) })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[--primary] focus:border-[--primary]"
                />
              </div>
              <div>
              ...
              {editedProgram.sessions?.map((session, sessionIndex) => (
                <div key={session.id} className="border rounded-lg p-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <input
                        ...
                        value={session.name}
                        onChange={(e) => {
                          const newSessions = [...(editedProgram.sessions || [])];
                          newSessions[sessionIndex] = { ...session, name: e.target.value };
                          setEditedProgram({ ...editedProgram, sessions: newSessions });
                        }}
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[--primary] focus:border-[--primary]"
                      />
                    </div>
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const newSessions = [...(editedProgram.sessions || [])];
                          newSessions[sessionIndex] = { ...session, day_of_week: parseInt(e.target.value) };
                          setEditedProgram({ ...editedProgram, sessions: newSessions });
                        }}
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[--primary] focus:border-[--primary]"
                      >
                  ...
                  <div className="space-y-4">
                    {session.training_exercises?.map((exercise, exerciseIndex) => (
                      <div key={exercise.id} className="bg-gray-50 rounded-lg p-4">
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            <h5 className="font-medium mb-2">{exercise.exercise?.title}</h5>
                            
                            {exercise.exercise_parameters?.map((param, paramIndex) => (
                              <div key={param.id} className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => {
                                      const newSessions = [...(editedProgram.sessions || [])];
                                      const newExercises = [...(newSessions[sessionIndex].training_exercises || [])];
                                      const newParams = [...(newExercises[exerciseIndex].exercise_parameters || [])];
                                      newParams[paramIndex] = { ...param, sets: parseInt(e.target.value) };
                                      newExercises[exerciseIndex] = { ...newExercises[exerciseIndex], exercise_parameters: newParams };
                                      newSessions[sessionIndex] = { ...newSessions[sessionIndex], training_exercises: newExercises };
                                      setEditedProgram({ ...editedProgram, sessions: newSessions });
                                    }}
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const newExercises = [...(newSessions[sessionIndex].training_exercises || [])];
                                      const newParams = [...(newExercises[exerciseIndex].exercise_parameters || [])];
                                      newParams[paramIndex] = { ...param, reps: parseInt(e.target.value) };
                                      newExercises[exerciseIndex] = { ...newExercises[exerciseIndex], exercise_parameters: newParams };
                                      newSessions[sessionIndex] = { ...newSessions[sessionIndex], training_exercises: newExercises };
                                      setEditedProgram({ ...editedProgram, sessions: newSessions });
                                    }}
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const newExercises = [...(newSessions[sessionIndex].training_exercises || [])];
                                      const newParams = [...(newExercises[exerciseIndex].exercise_parameters || [])];
                                      newParams[paramIndex] = { ...param, rest_seconds: parseInt(e.target.value) };
                                      newExercises[exerciseIndex] = { ...newExercises[exerciseIndex], exercise_parameters: newParams };
                                      newSessions[sessionIndex] = { ...newSessions[sessionIndex], training_exercises: newExercises };
                                      setEditedProgram({ ...editedProgram, sessions: newSessions });
                                    }}
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const newExercises = [...(newSessions[sessionIndex].training_exercises || [])];
                                      const newParams = [...(newExercises[exerciseIndex].exercise_parameters || [])];
                                      newParams[paramIndex] = { ...param, tempo: e.target.value };
                                      newExercises[exerciseIndex] = { ...newExercises[exerciseIndex], exercise_parameters: newParams };
                                      newSessions[sessionIndex] = { ...newSessions[sessionIndex], training_exercises: newExercises };
                                      setEditedProgram({ ...editedProgram, sessions: newSessions });
                                    }}
                                  ...
                                  <div className="col-span-2 md:col-span-4">
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        const newExercises = [...(newSessions[sessionIndex].training_exercises || [])];
                                        const newParams = [...(newExercises[exerciseIndex].exercise_parameters || [])];
                                        newParams[paramIndex] = { ...param, notes: e.target.value };
                                        newExercises[exerciseIndex] = { ...newExercises[exerciseIndex], exercise_parameters: newParams };
                                        newSessions[sessionIndex] = { ...newSessions[sessionIndex], training_exercises: newExercises };
                                        setEditedProgram({ ...editedProgram, sessions: newSessions });
                                      }}
            ...
            {userProgram ? (
              <div className="space-y-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-semibold">{userProgram.name}</h3>
                    <p className="text-sm text-gray-600">
                      {new Date(userProgram.start_date).toLocaleDateString()} - {new Date(userProgram.end_date).toLocaleDateString()}
                    </p>
                  </div>
                              ...
                              {exercise.exercise_parameters?.map((param) => (
                                <div key={param.id} className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                  <div>
                                    ...
                                    <span className="font-medium">{param.sets}</span>
                                  </div>
                                  <div>
                                    <span className="text-gray-600">Reps:</span>{' '}
                                    <span className="font-medium">{param.reps}</span>
                                  </div>
                                  <div>
                                    <span className="text-gray-600">Rest:</span>{' '}
                                    <span className="font-medium">{param.rest_seconds}s</span>
                                  </div>
                                  <div>
                                    <span className="text-gray-600">Tempo:</span>{' '}
                                    <span className="font-medium">{param.tempo}</span>
                                  </div>
                                  {param.notes && (
                                    <div className="col-span-2 md:col-span-4">
                                      <span className="text-gray-600">Notes:</span>{' '}
                                      <span className="font-medium">{param.notes}</span>
                                    </div>
                                  )}
                                </div>
                              ))}
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">No training program assigned yet.</p>
              </div>
            )}
          </div>
        )}
  ...
  const filteredAndSortedUsers = users
    .filter(user => {
      ...
      const matchesSubscription = 
        ...
        (filterSubscription === 'free' && user.subscription?.subscription_type === 'Free') ||
        (filterSubscription === 'paid' && user.subscription?.subscription_type !== 'Free');
      
      // Expiring program filter
      const matchesExpiringProgram = 
        !filterExpiringProgram || 
      ...
      return matchesSearch && matchesSubscription && matchesExpiringProgram && matchesExpiringSubscription;
    })
    .sort((a, b) => {
      let valueA, valueB;
      ...
      <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="p-4 bg-gray-50 rounded-lg">
                ...
                <ul className="space-y-2 text-gray-600">
                  ...
                  <li>Date of Birth: {userDetails.date_of_birth ? new Date(userDetails.date_of_birth).toLocaleDateString() : 'Not specified'}</li>
                  <li>Height: {userDetails.height || 'Not specified'} cm</li>
                  <li>Initial Weight: {userDetails.weight || 'Not specified'} kg</li>
                </ul>
              </div>
        ...
        <div className="card">
          ...
          {progressPhotos.length > 0 ? (
            ...
            <p className="text-center text-gray-600 py-8">No progress photos uploaded yet.</p>
          )}
        </div>
      </div>
    );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div className="flex items-end">
              <label className="flex items-center space-x-2 cursor-pointer">
                ...
                />
                <span className="text-sm text-gray-700">Expiring Programs (7 days)</span>
              </label>
            </div>
            ...
            <div className="flex items-end">
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setFilterExpiringSubscription(e.target.checked)}
                  className="h-4 w-4 text-[--primary] focus:ring-[--primary] border-gray-300 rounded"
                />
                <span className="text-sm text-gray-700">Expiring Subscriptions (7 days)</span>
              </label>
            </div>
          </div>
        </div>
